...
export const Navbar = () => {
  ...
  const startseiteRef = useRef<HTMLDivElement>(null);
  const referenzenRef = useRef<HTMLDivElement>(null);

  // Funktion zum Schließen der Dropdowns, wenn außerhalb geklickt wird
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (startseiteRef.current && !startseiteRef.current.contains(event.target as Node)) {
        setShowStartseiteDropdown(false);
      }
      if (referenzenRef.current && !referenzenRef.current.contains(event.target as Node)) {
        setShowReferenzenDropdown(false);
      }
    }
          ...
          <ul className="items-center justify-end flex-1 pt-6 list-none lg:pt-0 lg:flex">
            {navigation.map((menu, index) => (
              <li className="mr-3 nav__item" key={index}>
                {menu === "Startseite" ? (
                  <div className="relative" ref={startseiteRef}>
                    <button 
                      onClick={() => {
                        setShowStartseiteDropdown(!showStartseiteDropdown);
                        setShowReferenzenDropdown(false);
                      }}
                    ...
                    >
                      {menu}
                      <svg 
                        className={`w-4 h-4 ml-1 transition-transform ${showStartseiteDropdown ? 'rotate-180' : ''}`} 
                        fill="none" 
                        stroke="currentColor" 
                        viewBox="0 0 24 24" 
                        xmlns="http://www.w3.org/2000/svg"
                    ...
                    {showStartseiteDropdown && (
                      <div className="absolute left-0 z-10 pt-2">
                  ...
                  <div className="relative" ref={referenzenRef}>
                    <button 
                      onClick={() => {
                        setShowReferenzenDropdown(!showReferenzenDropdown);
                        setShowStartseiteDropdown(false);
                      }}
                      className="inline-block px-4 py-2 text-lg font-normal text-gray-800 no-underline rounded-md dark:text-gray-200 hover:text-indigo-500 focus:text-indigo-500 focus:bg-indigo-100 focus:outline-none dark:focus:bg-gray-800 flex items-center"
                    >
                      {menu}
                      <svg 
                        className={`w-4 h-4 ml-1 transition-transform ${showReferenzenDropdown ? 'rotate-180' : ''}`} 
                        fill="none" 
                        stroke="currentColor" 
                        viewBox="0 0 24 24" 
                        xmlns="http://www.w3.org/2000/svg"
